...
class SendCounter {
	public static void main(String[] args) {
        ...
        String strMailTo = "";	//收件者mail 人員改由DB中擷取資料	Wen 20110414
        String strContent = "";
...
//				EmployeeCount = Integer.parseInt(objRS.getString(1));	//Wen：不知道當初為什麼要寫得這麼複雜
            ...
            //---<---建議書使用率(首頁)------
            // 建議書使用率(首頁)〈國泰人壽業務〉每天
			if(debug)	writeLog("建議書使用率(首頁)〈國泰人壽業務〉每天 START");
			...
			if(debug)	writeLog("建議書使用率(首頁)〈國泰人壽業務〉每天 END");
            
            // 建議書使用率(首頁)〈國泰人壽業務〉每週
			if(debug)	writeLog("建議書使用率(首頁)〈國泰人壽業務〉每週 START");
			...
			if(debug)	writeLog("建議書使用率(首頁)〈國泰人壽業務〉每週 END");
            ...
            // 建議書使用率(首頁)〈國泰人壽業務〉每月
			if(debug)	writeLog("建議書使用率(首頁)〈國泰人壽業務〉每月 START");
			...
			if(debug)	writeLog("建議書使用率(首頁)〈國泰人壽業務〉每月 END");
            ...
            */
            ...
            //---<---編輯傳統型------
            // 新增修改傳統(CT)、利變(CB)型〈國泰人壽業務〉
			if(debug)	writeLog("新增修改傳統(CT)、利變(CB)型〈國泰人壽業務〉START");
            objPStmt = objConn.prepareStatement("select sum(cnt), count(distinct a.agidno) from insdb.sug_count a inner join insdb.web_basic b on a.agidno = b.ceempid where substr(b.cedvno,3,1) in ('1','4') and agidno!='' and in_day=(current date - 1 days) and isinv in ('CTI', 'CTU', 'CBI', 'CBU') with ur");	//sug_count的ISINV改為三碼控制：國壽(C)_傳統型(T)、利變型(B)_新增(I)修改(U)	Wen 20121212
            objRS = objPStmt.executeQuery();
            while (objRS.next()) {
			...
			if(debug)	writeLog("新增修改傳統(CT)、利變(CB)型〈國泰人壽業務〉每週 END");
            ...
            // 新增修改傳統(CT)、利變(CB)型〈國泰人壽業務〉每月
			if(debug)	writeLog("新增修改傳統(CT)、利變(CB)型〈國泰人壽業務〉每月 START");
            objPStmt = objConn.prepareStatement("select sum(a.cnt), count(distinct a.agidno), count(distinct a.in_day) from insdb.sug_count a inner join insdb.web_basic b on a.agidno = b.ceempid where substr(b.cedvno,3,1) in ('1','4') and a.agidno!='' and DAYOFWEEK_ISO(a.in_day) <6  and a.in_day >= (current date - 30 days) and a.in_day < current date and isinv in ('T', 'CT', 'CB', 'CTI', 'CTU', 'CBI', 'CBU') with ur");
			objRS = objPStmt.executeQuery();
			while (objRS.next()) {
			...
			if(debug)	writeLog("新增修改傳統(CT)、利變(CB)型〈國泰人壽業務〉每月 END");
            ...
            */
            ...
            //---<---編輯投資型------
            // 新增修改投資型〈國泰人壽業務〉
			if(debug)	writeLog("新增修改投資型〈國泰人壽業務〉 START");
            objPStmt = objConn.prepareStatement("select sum(cnt), count(*) from insdb.sug_count a inner join insdb.web_basic b on a.agidno = b.ceempid where substr(b.cedvno,3,1) in ('1','4') and agidno!='' and in_day=(current date - 1 days) and isinv in ('CII', 'CIU') with ur");	//sug_count的ISINV改為三碼控制：國壽(C)_投資型(I)_新增(I)、修改(U)	Wen 20121212
            objRS = objPStmt.executeQuery();
            while (objRS.next()) {
			...
			if(debug)	writeLog("新增修改投資型〈國泰人壽業務〉 END");
            
            // 新增修改投資型〈國泰人壽業務〉每週
			if(debug)	writeLog("新增修改投資型〈國泰人壽業務〉每週  START");
            objPStmt = objConn.prepareStatement("select sum(a.cnt), count(distinct a.agidno), count(distinct a.in_day) from insdb.sug_count a inner join insdb.web_basic b on a.agidno = b.ceempid where substr(b.cedvno,3,1) in ('1','4') and a.agidno!='' and DAYOFWEEK_ISO(a.in_day) <6  and a.in_day >= (current date -  7 days) and a.in_day < current date and isinv in ('I', 'CI', 'CII', 'CIU') with ur");
			objRS = objPStmt.executeQuery();
			while (objRS.next()) {
			...
			if(debug)	writeLog("新增修改投資型〈國泰人壽業務〉每週  END");
            ...
            // 新增修改投資型〈國泰人壽業務〉每月
			if(debug)	writeLog("新增修改投資型〈國泰人壽業務〉每月  START");
            objPStmt = objConn.prepareStatement("select sum(a.cnt), count(distinct a.agidno), count(distinct a.in_day) from insdb.sug_count a inner join insdb.web_basic b on a.agidno = b.ceempid where substr(b.cedvno,3,1) in ('1','4') and a.agidno!='' and DAYOFWEEK_ISO(a.in_day) <6  and a.in_day >= (current date - 30 days) and a.in_day < current date and isinv in ('I', 'CI', 'CII', 'CIU') with ur");
			objRS = objPStmt.executeQuery();
			while (objRS.next()) {
			...
			if(debug)	writeLog("新增修改投資型〈國泰人壽業務〉每月 END");
...
//      String mailserver   = "cathaylife.com.tw";
        ...
        */
        ...
        if (blnIsException == true) {
			...
			writeLog(strDateOutput + ":測試crm mail cc compile！");
